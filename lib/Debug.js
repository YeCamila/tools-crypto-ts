/**
 * crypto Debug
 * https://github.com/YeCamila/tools-crypto-ts
 */
"use strict";

// export DEBUG=crypto:*,info / unset DEBUG
// localStorage.debug = 'crypto:*';
...
const $toJson = (v) => { return JSON.stringify(v,null,2); };
...
const $ab2arr = (buf) => { 
	if (!$is(buf,ArrayBuffer)) return null;
	return Array.from(new Uint8Array(buf));
};

const $arr2ab = (arr) => { 
	if (!Array.isArray(arr)) return null;
	if (!Number.isInteger(arr[0])) return null;
...
// colors
	...
	trim(arr) { return $trim(arr); }

	toString(obj){ return $toString(obj); }

	fromString(s){ return $fromString(s); }

	ab2arr(buf){ return $ab2arr(buf); }

	arr2ab(arr){ return $arr2ab(arr); }
